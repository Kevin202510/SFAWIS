...
abstract class BaseEnum
{
    ...
    public static function modelingsExceptById($id)
    {
        return array_filter(static::modelings(), function ($value) use ($id) {
            return $value['id'] != $id;
        });
    }
    ...
    public static function labelsOf($all = [], $translate = true): array
    {
        ...
        foreach ($all as $value) {
            $label = $definedLabels[$value] ?? str_replace('_', ' ', subject: Str::title(static::findConstantName($value)));
